...
func TestCreateTransaction(t *testing.T) {
	...
	client2, _ := NewClient("John Doe 1", "j@j2")
	account2 := NewAccount(client2)

	account1.Credit(1000)
	account2.Credit(1000)

	transation, err := NewTransaction(account1, account2, 100)
	assert.Nil(t, err)
...
func TestCreateTransactionWithInsuficientBalance(t *testing.T) {
	...
	transation, err := NewTransaction(account1, account2, 2000)
	assert.NotNil(t, err)
	assert.Error(t, err, "insufficient funds")
	assert.Nil(t, transation)
...
func TestCreateTransactionWithNegativeBalance(t *testing.T) {
	...
	transation, err := NewTransaction(account1, account2, -1000)
	assert.NotNil(t, err)
